...
def reverse_recursive(lst):
    ...
    >>> print("Do not use lst[::-1], lst.reverse(), or reversed(lst)!") if any([r in cleaned for r in ["[::", ".reverse", "reversed"]]) else None
    """
    def new_old_lst(new: list, old: list):
        if len(old) == 0:
            return new
        add_=old[-1]
        new.append(add_)
        old.pop(-1)
        return new_old_lst(new,old)
    return new_old_lst([], lst)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    new_place = make_city('new', lat, lon)
    distance1 = distance(new_place, city_a)
    distance2 = distance(new_place, city_b)
